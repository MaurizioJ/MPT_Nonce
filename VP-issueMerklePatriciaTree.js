...
const test = async (accounts) => {
    ...
    for (let i = j; i <j+1; i++) {
        ...
        vcCreationTimes.push([res/config.merklePatriciaTree.iterations, jwtSize/config.merklePatriciaTree.iterations]);
        console.log("-------------------------------------------------------------------------")
    ...
    file.appendFile('test_Creation_VP'+soglia+'_NOcomp.txt', Math.pow(2,j) + ' ' + vcCreationTimes + '\n', function (err) {
        if (err) throw err;
